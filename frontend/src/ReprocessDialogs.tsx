...
function ReprocessDialog({ preflight, project, submitting, uncertain, reconfirm, error, close, adjust, act, confirm, start }: { preflight: ReprocessPreflight; project: ProjectSummary; submitting: boolean; uncertain: boolean; reconfirm: boolean; error: string; close(): void; adjust(): void; act(action: ReprocessBlockerAction): void; confirm(): void; start(): void }) {
  ...
  return <DialogFrame wide closeDisabled={submitting} onClose={close} title="重新处理这条录像" description="将使用当前设置创建新的处理记录。原有记录和成片不会被修改。" footer={<><button className="button" disabled={submitting} onClick={close}>取消</button><span className="footer-spacer" />{!blocked && <button className="button" disabled={submitting} onClick={adjust}>调整设置</button>}{reconfirm && <button className="button primary" onClick={confirm}>确认最新检查结果</button>}<button className="button primary" disabled={blocked || submitting || reconfirm} onClick={start}>{submitting ? "正在创建…" : "开始重新处理"}</button></>}>
    ...
    <div className="reprocess-summary-grid"><section><span>处理对象</span><h2>{preflight.source.name}</h2><dl><div><dt>所属项目</dt><dd>{project.name}</dd></div><div><dt>来源版本</dt><dd>{versionLabel(preflight.run.processing_sequence)}</dd></div><div><dt>录像状态</dt><dd>{preflight.source.state === "ready" ? "原文件可用" : "需要修复"}</dd></div></dl><p>不会修改原文件；处理时会在受控工作目录创建临时工作副本。</p></section><section><span>当前设置变化</span>{preflight.changes.length ? <dl>{preflight.changes.map((item) => <div key={item.field}><dt>{FIELD_LABELS[item.field]}</dt><dd><small>{value(item.before)}</small><b>至</b><strong>{value(item.after)}</strong></dd></div>)}</dl> : <p>当前设置与来源版本相同。</p>}</section></div>
    ...
    <div className="reprocess-facts"><div><span>工作空间</span><strong>{bytes(preflight.space.available_bytes)} 可用</strong><small>预计至少需要 {bytes(preflight.space.required_bytes)}</small></div><div><span>输出目录</span><strong>{value(preflight.current_settings.summary.output_directory)}</strong><small>{preflight.space.sufficient ? "空间检查通过" : "空间不足"}</small></div><div><span>原有结果</span><strong>保留</strong><small>处理记录、成片和发布物料不会被覆盖</small></div></div>
    {uncertain && <p className="reprocess-uncertain">关闭或重载后再次进入，仍会复用本次请求编号。</p>}
  </DialogFrame>;
}
