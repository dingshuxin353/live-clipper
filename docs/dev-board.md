...
| 车道 | Spec | 文件集合 | 依赖 | 状态 |
|---|---|---|---|---|
| 0.3.1-model-download-foundation | `specs/2026-07-24-0.3.1-model-download-foundation.md` | 模型下载、两源配置、设置页与相关测试 | `master@bef3f82` | v1.2 提交 `415a4c1` 已通过独立验收，等待本地合并 |
...
| 顺序 | 车道 | Spec | 依赖 | 状态 |
|---:|---|---|---|---|
| 1 | 0.3.1-misans-default-font Phase 2 | 同一字体 Spec 与 lane | 字体 Phase 1 + 下载基础设施验收并合入 | 接线门禁，禁止提前开工 |
| 2 | 0.3.1-model-matrix-selection | `specs/2026-07-24-0.3.1-model-matrix-selection.md` | 下载基础设施与字体 Phase 2 合入后回写精确基线 | 排队，禁止提前开工 |
...
- **0.3.1 · 本地模型与首次使用闭环**：Qwen3 不进入本版本；下载基础设施已派发，MiSans 字体资产与 macOS 菜单栏图标修复可并行、CSS 接线在下载基础设施合入后继续，随后实施 Whisper 三档/当前模型闭环；其后再规划首启向导、tag 自动工作流停用、本地 `.[mlx]` 正式构建、真实中文转写与 `0.3.0 → 0.3.1` 自动更新真机演练。
