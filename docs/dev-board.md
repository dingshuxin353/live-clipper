...
| 车道 | Spec | 文件集合 | 依赖 | 状态 |
|---|---|---|---|---|
| 0.3.1-misans-default-font | `specs/2026-07-24-0.3.1-misans-default-font.md` | Phase 1 仅新增字体、许可、notice 与资产测试 | 以 Spec 头部精确基线为准 | Phase 1 已提交 `aa28794`，等待独立验收；Phase 2 前置下载基础设施已合入，仍等待 Phase 1 独立验收 |
...
| 顺序 | 车道 | Spec | 依赖 | 状态 |
|---:|---|---|---|---|
| 1 | 0.3.1-misans-default-font Phase 2 | 同一字体 Spec 与 lane | 字体 Phase 1 独立验收通过；下载基础设施已合入 | 仍等待字体 Phase 1 独立验收，禁止提前开工 |
| 2 | 0.3.1-model-matrix-selection | `specs/2026-07-24-0.3.1-model-matrix-selection.md` | 下载基础设施与字体 Phase 2 合入后回写精确基线 | 下载基础设施已合入，仍等待字体 Phase 2 合入 |
...
- **0.3.1 · 本地模型与首次使用闭环**：Qwen3 不进入本版本；下载基础设施已合入，MiSans 字体资产与 macOS 菜单栏图标修复可并行、CSS 接线在字体 Phase 1 独立验收后继续，随后实施 Whisper 三档/当前模型闭环；其后再规划首启向导、tag 自动工作流停用、本地 `.[mlx]` 正式构建、真实中文转写与 `0.3.0 → 0.3.1` 自动更新真机演练。
