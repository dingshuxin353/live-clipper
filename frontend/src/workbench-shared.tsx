...
export const STAGES: Array<[RunStage, string]> = [["read_source", "读取录像"], ["transcribe", "语音转写"], ["analyze", "内容分析"], ["arbitrate", "结果仲裁"], ["review", "人工审阅"], ["render", "渲染成片"]];
export const STATUS_LABELS: Record<ProjectMainStatus, string> = { blocked: "需要处理", failed: "处理失败", awaiting_review: "等待审阅", processing: "处理中", queued: "排队中", paused: "已暂停", inactive: "未启用", idle: "空闲" };
...
export interface ProjectDraft {
  ...
  asrRef: string; analysisRef: string; retention: ProjectConfig["output"]["intermediate_retention"];
}
...
export function configFromDraft(draft: ProjectDraft, timezone: string): ProjectConfig {
  return {
    ...
    resources: { asr_ref: draft.asrRef, analysis_ref: draft.analysisRef, arbitration_mode: "reuse_analysis", arbitration_ref: null },
...
export function RunCard({ run, project }: { run: Run; project?: ProjectSummary }) { return <Link className="run-card" to={`/projects/${run.project_id}/runs/${run.run_id}`}><div><span className="overline">{project?.name ?? "项目"}</span><strong>{basename(run.latest_seen_path)}</strong><small>{time(run.updated_at)}</small></div><StatusPill status={run.status} label={run.status === "queued" && run.queue_position ? `队列第 ${run.queue_position} 位` : undefined} /></Link>; }

const PRIORITY: Record<ProjectMainStatus, number> = { blocked: 0, failed: 1, awaiting_review: 2, processing: 3, queued: 4, paused: 5, inactive: 6, idle: 7 };
...
export function ProjectRow({ project, detailed = false }: { project: ProjectSummary; detailed?: boolean }) { const next = project.schedule?.enabled ? `下次扫描 ${time(project.schedule.next_scan_at)}` : "仅手动扫描"; return <Link className="project-row" to={`/projects/${project.project_id}`}><span className={`project-indicator tone-${statusTone(project.main_status)}`} /><div className="project-copy"><strong>{project.name}</strong><p>{project.description || "未填写项目描述"}</p>{detailed && <small>{next} · 更新于 {time(project.updated_at)}</small>}</div><div className="workload"><span>{project.workload.processing}<small>处理中</small></span><span>{project.workload.queued}<small>排队</small></span><span>{project.workload.awaiting_review}<small>待审</small></span><span>{project.workload.failed}<small>失败</small></span><span>{project.workload.completed}<small>已完成</small></span></div><StatusPill status={project.main_status} /><b>›</b></Link>; }
