...
export function Onboarding({ snapshot, onSession, onRefresh, onPaused, onClose }: Props) {
  ...
  return <Dialog isOpen role="dialog" purpose={busy || trialOpen ? "required" : "form"} onOpenChange={open => { if (!open) void pause(); }} width={1120} maxHeight="94dvh" padding={0} aria-labelledby="onboarding-title"><section className="onboarding-shell">
    ...
    <div className="onboarding-layout"><aside className="onboarding-steps" aria-label="首次设置步骤">{STEPS.map((item, index) => <Button key={item.id} isDisabled={index > stepIndex || session.state !== "in_progress"} onClick={() => index < stepIndex && void go(item.id)} className="onboarding-step-button" label={item.label} variant={index === stepIndex ? "primary" : "secondary"} icon={<span>{index < stepIndex ? <RemixIcon name="check" /> : index + 1}</span>}><span className="onboarding-step-text"><span>{item.label}</span><small>{item.note}</small></span></Button>)}<p><strong>设置自动保留</strong><span aria-live="polite">{saveState === "saving" ? "正在保存非密钥设置…" : saveState === "failed" ? "自动保存失败，请处理后重试。" : "非密钥设置已保存。"}</span><br />关闭窗口或稍后继续，不会删除已下载模型和已提交配置。</p></aside>
