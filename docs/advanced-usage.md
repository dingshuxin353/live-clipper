...
`live-clipper` 的源码流程会提取音频、生成带时间戳的转录、调用 OpenAI-compatible LLM 校对和扫描候选、生成审阅包，再根据人工或 Agent 写入的选择用 `ffmpeg` 渲染字幕与短视频。

默认产物写在本地目录。配置云端 ASR 时，音频会发送到对应服务；配置云端 LLM 或 AI 审阅时，完成判断所需的转录文本会发送到对应服务。详见[隐私说明](privacy.md)。

## 小白推荐：让 AI 陪你配置

如果你不熟悉 Python、终端或配置文件，建议先走这条路径：

1. 打开 [AI 使用说明](ai-assistant-guide.md)。
2. 把全文复制给你常用的 AI 助手。
...
如果你想使用当前 MCP 工作台版本，尤其是想知道“如何让 AI 选片”，请看 [MCP 工作台使用指南](mcp-workbench-user-guide.md)。
...
桌面版无需命令行：设置页可直接下载本地语音模型，支持切换下载源（ModelScope 中国大陆推荐 / Hugging Face 国际官方），下载进度实时显示，模型存放在应用数据目录的 `models/` 子目录，删除后可随时重新下载。
...
这会生成 `live-clipper.toml`。普通配置只放在 TOML；`.env` 只存 API key 等密钥。API key 不写入 TOML，也不会写入日志正文。
...
当前运行会在 `run_metadata.json` 中记录提示词来源。详见[提示词说明](prompts.md)。
...
- `文件清理`：查看 MCP/Web 创建的删除确认请求，支持单条确认/拒绝和批量确认/拒绝。
- `日志`：查看事件流和任务日志尾部。
...
- `基础设置`：只需回答录像在哪、成片放哪、AI 用哪家，并选择语音识别方式和是否自动生成成片。
- `自动化`：按时间表自动扫描和检查，也可以明确开启 AI 自动选片。
- `高级设置（一般不需要改）`：集中管理存储与扫描、语音识别、模型请求、服务与调度、AI 审阅参数和 Web 控制台。
- `检查配置`：只校验当前表单，不写入文件。
...
- `自动化`：用人话管理按时间表自动扫描和检查、AI 自动选片、审阅方式和定时任务。
- `高级设置（一般不需要改）`：默认折叠；存储与扫描、语音识别、模型请求、服务与调度、AI 审阅参数和 Web 控制台仍可展开编辑。

保存、检查、重载、恢复默认和重启服务仍使用同一套配置 API；收起的高级字段也会随当前表单一起保留，不会因为只看基础设置而丢失。页面仍不会展示明文 API key，删除和清理动作继续走 confirmation，AI 审阅也不会直接写文件。
...
- 版本与安装包：[GitHub Releases](https://github.com/dingshuxin353/live-clipper/releases)
- 许可证：[LICENSE](../LICENSE)
