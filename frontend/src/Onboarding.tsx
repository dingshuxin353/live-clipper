...
function Welcome({ environment, busy, recheck, next, pause }: { environment: OnboardingEnvironment; busy: string; recheck(): void; next(): void; pause(): void | Promise<void> }) {
  ...
  return <div className="onboarding-step"><div className="onboarding-scroll"><span className="onboarding-eyebrow">欢迎使用 Venus</span><h2>完成首次设置</h2><p>设置语音识别、AI 服务和第一个项目后，即可开始处理录像。</p><div className="onboarding-flow"><div><b>1</b><strong>设置处理资源</strong></div><i>→</i><div><b>2</b><strong>创建项目</strong></div><i>→</i><div><b>3</b><strong>处理录像</strong></div></div><section className="onboarding-checks"><header><div><strong>{environment.status === "ready" ? "运行环境已准备好" : "运行环境需要处理"}</strong><p>检查 Venus 运行所需的本机环境。</p></div><button className="button" data-busy={busy === "environment" ? "true" : undefined} disabled={busy === "environment"} onClick={recheck}>{busy === "environment" ? "检查中…" : "重新检查"}</button></header>{groups.map((group) => { const checks = environment.checks.filter((item) => group.names.includes(item.name)); const ready = checks.every((item) => item.status === "ready"); return <div key={group.label} className={ready ? "ready" : "blocked"}><span>{ready ? "✓" : "!"}</span><strong>{group.label}</strong><small>{ready ? "已就绪" : checks.find((item) => item.problem)?.problem || "需要检查"}</small></div>; })}</section></div><StepFooter pause={pause} action={next} label="开始设置" disabled={environment.status !== "ready"} note="模型下载中断后可以继续" /></div>;
}
...
function AiStep({ snapshot, draft, status, busy, error, keyInput, keyRef, update, submit, back, next, pause }: { snapshot: OnboardingSnapshot; draft: OnboardingDraft; status: string; busy: string; error: string; keyInput: React.RefObject<HTMLInputElement | null>; keyRef: React.MutableRefObject<string>; update: <S extends keyof OnboardingDraft>(section: S, field: string, value: string | number) => void; submit(): void; back(): void; next(): void; pause(): void | Promise<void> }) {
  ...
  return <div className="onboarding-step"><div className="onboarding-scroll"><span className="onboarding-eyebrow">AI 服务</span><h2>连接 AI 服务</h2><p>内容分析、AI 判断和发布物料会使用这个服务。</p><section className="onboarding-connection"><div className="onboarding-providers">{snapshot.provider_presets.map((item) => <button key={item.id} className={providerId === item.id ? "selected" : ""} onClick={() => choose(item.id)}><strong>{item.label}</strong><small>{item.id === "custom" ? "兼容接口" : "填入建议配置"}</small></button>)}</div><label>服务地址<input value={draft.ai?.api_base || ""} aria-describedby={error ? "onboarding-action-error" : undefined} onChange={(event) => update("ai", "api_base", event.target.value)} /></label><label>模型<input value={draft.ai?.model || ""} onChange={(event) => update("ai", "model", event.target.value)} /></label><label>API key<input ref={keyInput} className="onboarding-secret-input" type="password" autoComplete="off" aria-invalid={error.includes("API key")} aria-errormessage={error ? "onboarding-action-error" : undefined} onInput={(event) => { keyRef.current = event.currentTarget.value; }} /></label><div className={`onboarding-test ${status}`}><div><strong>{status === "ready" ? "AI 服务连接成功" : status === "error" ? "连接需要修改" : "连接尚未验证"}</strong><p>{status === "ready" ? "已保存凭据" : "发送测试请求，检查服务、凭据和模型。"}</p></div><button className="button" data-busy={busy === "ai" ? "true" : undefined} disabled={busy === "ai"} onClick={submit}>{busy === "ai" ? "测试中…" : "测试并保存"}</button></div></section><div className="onboarding-shared-service"><strong>内容分析、AI 判断和发布物料</strong><span>使用同一个已验证服务</span></div></div><VisuallyHidden as="div" aria-atomic="true" aria-live="polite" role="status">{busy === "ai" ? "正在测试 AI 服务连接" : ""}</VisuallyHidden><StepFooter back={back} pause={pause} action={next} label="继续" disabled={status !== "ready"} note={status === "ready" ? "连接信息已保存" : "连接通过后才能继续"} /></div>;
}
function ProjectStep({ draft, snapshot, busy, update, markNameEdited, selectFolder, back, validate, pause }: { draft: OnboardingDraft; snapshot: OnboardingSnapshot; busy: string; update: <S extends keyof OnboardingDraft>(section: S, field: string, value: string | number) => void; markNameEdited(): void; selectFolder(kind: "source" | "output"): void; back(): void; validate(): void; pause(): void | Promise<void> }) {
  ...
  return <div className="onboarding-step"><div className="onboarding-scroll"><span className="onboarding-eyebrow">第一个项目</span><h2>设置第一个项目</h2><p>选择录像目录、成片位置和扫描方式。</p><section className="onboarding-project-form"><label>项目名称<input value={project.name || ""} onChange={(event) => { markNameEdited(); update("project", "name", event.target.value); }} /></label><label>发现新录像<select value={project.trigger_mode || "manual"} onChange={(event) => update("project", "trigger_mode", event.target.value)}><option value="manual">仅手动扫描</option><option value="scheduled">定时扫描 + 手动扫描</option></select></label><label className="span-two">录像目录<div className="onboarding-folder"><input value={project.source_directory || ""} onChange={(event) => update("project", "source_directory", event.target.value)} /><button className="button" type="button" onClick={() => void selectFolder("source")}>选择…</button></div><small>{project.source_directory ? "检查时会读取目录并统计已有录像" : "选择存放直播录像的文件夹"}</small></label>{scheduled && <><label>定时方式<select value={project.schedule_mode || "daily"} onChange={(event) => update("project", "schedule_mode", event.target.value)}><option value="daily">每天固定时间</option><option value="interval">固定间隔</option></select></label>{project.schedule_mode === "interval" ? <label>扫描间隔<select value={project.interval_minutes || 60} onChange={(event) => update("project", "interval_minutes", Number(event.target.value))}><option value={30}>每 30 分钟</option><option value={60}>每 1 小时</option><option value={180}>每 3 小时</option><option value={360}>每 6 小时</option><option value={720}>每 12 小时</option></select></label> : <label>扫描时间<input type="time" value={project.daily_time || "22:00"} onChange={(event) => update("project", "daily_time", event.target.value)} /></label>}</>}<label className="span-two">成片保存位置<div className="onboarding-folder"><input value={project.output_directory || ""} onChange={(event) => update("project", "output_directory", event.target.value)} /><button className="button" type="button" onClick={() => void selectFolder("output")}>选择…</button></div></label></section><div className="onboarding-defaults"><div><span>已准备</span><strong>{snapshot.resources.asr.model_label || "语音识别"}与 {snapshot.resources.ai.model || "AI 服务"}</strong></div><div><span>处理方式</span><strong>AI 自动判断并生成成片，默认只处理创建后新增的录像</strong></div><div><span>文件处理</span><strong>原始录像不会自动删除，成片与临时文件分开保存</strong></div></div></div><StepFooter back={back} pause={pause} action={validate} label={busy === "validate" ? "检查中…" : "检查配置"} disabled={busy === "validate" || !project.name || !project.source_directory || !project.output_directory} note="继续前会检查目录和处理资源" /></div>;
}
