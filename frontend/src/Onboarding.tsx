...
function AiStep({ snapshot, draft, status, busy, error, keyInput, keyRef, update, submit, back, next, pause }: { snapshot: OnboardingSnapshot; draft: OnboardingDraft; status: string; busy: string; error: string; keyInput: React.RefObject<HTMLInputElement | null>; keyRef: React.MutableRefObject<string>; update: <S extends keyof OnboardingDraft>(section: S, field: string, value: string | number) => void; submit(): void; back(): void; next(): void; pause(): void | Promise<void> }) {
  ...
  return <div className="onboarding-step"><div className="onboarding-scroll"><span className="onboarding-eyebrow">AI 服务</span><h2>连接 AI 服务</h2><p>内容分析、AI 判断和发布物料会使用这个服务。</p><FormLayout className="onboarding-connection form-surface"><div className="onboarding-providers">{snapshot.provider_presets.map((item) => <button key={item.id} className={providerId === item.id ? "selected" : ""} onClick={() => choose(item.id)}><strong>{item.label}</strong><small>{item.id === "custom" ? "兼容接口" : "填入建议配置"}</small></button>)}</div><FormLayout className="form-pair"><TextInput label="服务地址" onChange={(value) => update("ai", "api_base", value)} value={draft.ai?.api_base || ""} width="100%" /><TextInput label="模型" onChange={(value) => update("ai", "model", value)} value={draft.ai?.model || ""} width="100%" /></FormLayout><Field inputID="onboarding-ai-api-key" label="API key" width="100%"><input ref={keyInput} className="form-control form-secret-input" id="onboarding-ai-api-key" type="password" autoComplete="off" aria-invalid={error.includes("API key")} aria-errormessage={error ? "onboarding-action-error" : undefined} onInput={(event) => { keyRef.current = event.currentTarget.value; }} /></Field><div className={`onboarding-test ${status}`}><div><strong>{status === "ready" ? "AI 服务连接成功" : status === "error" ? "连接需要修改" : "连接尚未验证"}</strong><p>{status === "ready" ? "已保存凭据" : "发送测试请求，检查服务、凭据和模型。"}</p></div><button className="button" data-busy={busy === "ai" ? "true" : undefined} disabled={busy === "ai"} onClick={submit}>{busy === "ai" ? "测试中…" : "测试并保存"}</button></div></FormLayout><div className="onboarding-shared-service"><strong>内容分析、AI 判断和发布物料</strong><span>使用同一个已验证服务</span></div></div><VisuallyHidden as="div" aria-atomic="true" aria-live="polite" role="status">{busy === "ai" ? "正在测试 AI 服务连接" : ""}</VisuallyHidden><StepFooter back={back} pause={pause} action={next} label="继续" disabled={status !== "ready"} note={status === "ready" ? "连接信息已保存" : "连接通过后才能继续"} /></div>;
}
