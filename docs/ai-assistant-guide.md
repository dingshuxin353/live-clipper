# live-clipper 源码与命令行兼容流程 AI 使用说明

这份说明面向从源码或命令行使用 live-clipper 的高级用户，不是 Venus 1.0.0 桌面客户端的首次设置流程。你可以把它复制给常用的 AI 助手，让它用中文协助初始化、配置、第一次运行和 Agent 定时任务配置。
...
先判断用户使用 Venus 桌面客户端还是源码/CLI：桌面客户端在首次设置中选择语音识别方式，源码/CLI 默认使用本地 MLX。AI 与云端 ASR 的 key 用途不同，都不得发到聊天窗口。源码/CLI 用户首次运行本机模型可能需要下载模型，耗时较长。
...
- 先判断用户使用桌面客户端还是源码/CLI；桌面客户端在首次设置中选择语音识别方式，源码/CLI 默认使用本地 MLX。AI 与云端 ASR 的 key 用途不同，都不得发到聊天窗口。
...
本节适用于默认使用本地 MLX 的源码/CLI 模式。桌面客户端在首次设置中选择语音识别方式；两种模式已有配置都不会被自动覆写。源码/CLI 的默认模型是 `mlx-community/whisper-large-v3-turbo`。
