...
const STAGES = [
  ["copy", "创建安全备份"], ["project", "建立默认项目"], ["history", "导入历史记录"],
...
export function MigrationFlow({ startup, onEnter }: Props) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      catch (caught) { if (!stopped && !(caught instanceof DOMException && caught.name === "AbortError")) setConnection("暂时无法刷新，正在保留上一次真实进度"); }
  ...
  async function validate() {
    ...
    try { const result = await projectApi.migrationValidate(inspectionPlan.source_fingerprint, inspectionPlan.plan_hash, normalizedChoices(choices)); setValidatedPlan(result.plan); setChoices(result.plan.choices); setLocalScreen("confirm"); }
      ...
      else { setError(message(caught, "升级方案未通过检查")); setErrorId(diagnosticId(caught)); if (caught instanceof ApiError) setFields(caught.fields); }
    } finally { setBusy(""); }
  }
  ...
  async function acknowledge() {
    ...
    try { const result = await projectApi.migrationAcknowledge(acknowledgeId.current, session.migration_id, session.revision); onEnter(result.project_id); }
      ...
      else { setError(message(caught, "暂时无法进入默认项目")); setErrorId(diagnosticId(caught)); }
    } finally { setBusy(""); }
  }
  ...
  return <div className="migration-layer"><section className="migration-shell" role="dialog" aria-modal="true" aria-labelledby="migration-title" ref={dialogRef}>
    <header className="migration-header"><div className="migration-brand"><img src="/static/venus-mark.png" alt="" /><strong>Venus</strong></div><div><span>安全升级</span><small>{active ? "升级正在本机执行，请保持 Venus 运行" : "先备份，再切换；原始录像不会被修改"}</small></div><span className={`migration-safety ${active ? "active" : ""}`}>{active ? "执行中" : "本机处理"}</span></header>
    <div className="migration-layout"><aside className="migration-steps" aria-label="升级步骤">{STEPS.map(([label, note], index) => <div className={index === step ? "active" : index < step ? "done" : ""} key={label}><span>{index < step ? "✓" : index + 1}</span><div><strong>{label}</strong><small>{note}</small></div></div>)}<p><strong>安全边界</strong>只读取旧版设置与记录；不会扫描或删除原始录像。</p></aside>
...
function Loading() { return <div className="migration-loading" role="status"><span className="migration-spinner" /><strong>正在读取升级状态</strong><p>正在确认旧版数据与已完成的安全检查…</p></div>; }
function CheckStep({ source, busy, inspect, quit, quitAvailable, error, titleRef }: { source: MigrationSnapshot["source"] | null; busy: string; inspect(): void; quit(): void; quitAvailable: boolean; error: string; titleRef: RefObject<HTMLHeadingElement | null> }) {
  return <div className="migration-step"><div className="migration-scroll"><span className="migration-eyebrow">开始之前</span><h1 id="migration-title" ref={titleRef} tabIndex={-1}>检查现有内容，准备安全升级</h1><p>Venus 检测到旧版设置和工作记录。检查过程只读，不会创建备份、项目或修改现有数据。</p><section className="migration-source-card"><div><span>已检测到</span><strong>{source ? `${source.display_summary.metadata_file_count} 组设置与记录文件` : "旧版数据"}</strong><small>{source?.display_summary.history_count !== undefined ? `${source.display_summary.history_count} 条历史记录等待检查` : "检查后会显示历史处置结论"}</small></div><span className="status-pill ready">只读检查</span></section><details className="migration-details"><summary>哪些内容会被读取</summary><p>只读取 Venus 旧版配置、运行记录和调度状态，用于生成迁移计划。不会读取媒体正文，不会把本机信息发送到外部服务。</p></details>{error && <p className="migration-safe-note">检查未完成；尚未创建备份或修改现有数据。</p>}</div><footer className="migration-footer"><button className="button" disabled={!quitAvailable || Boolean(busy)} onClick={quit}>退出 Venus</button><span /><small>检查完成后，你可以先核对全部方案再决定是否升级。</small><button className="button primary" disabled={Boolean(busy)} onClick={inspect}>{busy === "inspect" ? "检查中…" : error ? "重新检查" : "检查升级内容"}</button></footer></div>;
}
function DifferenceStep({ plan, choices, fields, busy, historyLimit, update, select, more, back, next, titleRef }: { plan: MigrationPlan; choices: MigrationChoices; fields: Record<string, string>; busy: string; historyLimit: number; update(value: Partial<MigrationChoices>): void; select(field: "source_directory" | "output_directory"): void; more(): void; back(): void; next(): void; titleRef: RefObject<HTMLHeadingElement | null> }) {
  ...
  return <div className="migration-step"><div className="migration-scroll"><span className="migration-eyebrow">升级方案</span><h1 id="migration-title" ref={titleRef} tabIndex={-1}>{required.size ? "处理必要差异" : "升级内容已准备好"}</h1><p>{required.size ? "只需确认下面标出的差异，其余内容已形成安全默认方案。" : "请核对默认项目、处理能力、历史记录和备份空间。"}</p>
